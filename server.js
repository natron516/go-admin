...
app.post('/webhooks/mux', express.raw({ type: 'application/json' }), async (req, res) => {
  ...
  if (event.type === 'video.asset.live_stream_completed') {
    ...
    try {
      ...
      // ── Sermon thumbnail: check Firebase Storage for a pre-generated thumb ──
      // Thumbnails are generated locally (scripts/generate-sermon-thumbs.js)
      // and uploaded to Firebase Storage AHEAD of time. On stream end, check if
      // one already exists for today and set pt.thumbnail immediately — no delay.
      // Cache-bust rule: if we're overwriting an existing URL, bump ?v= counter.
      if (catLabel === 'Sermon' && sa) {
        (async () => {
          try {
            const isoDate = new Date(asset.created_at * 1000).toISOString().slice(0, 10);
            const storagePath = `sermon-thumbs/sermon_${isoDate}.jpg`;
            const bucket = admin.storage().bucket();
            const [exists] = await bucket.file(storagePath).exists();
            if (exists) {
              // Re-read current pt (may have changed above)
              const cur = await mux('GET', `/video/v1/assets/${assetId}`);
              const curPt = parsePassthrough(cur.data?.passthrough || asset.passthrough);
              const baseUrl = `https://storage.googleapis.com/${bucket.name}/${storagePath}`;
              // Increment ?v= cache-bust if overwriting
              const existing = curPt.thumbnail || '';
              let newUrl = baseUrl;
              if (existing && existing.startsWith(baseUrl)) {
                const vMatch = existing.match(/\?v=(\d+)$/);
                const v = vMatch ? parseInt(vMatch[1], 10) + 1 : 2;
                newUrl = `${baseUrl}?v=${v}`;
              }
              curPt.thumbnail = newUrl;
              await mux('PATCH', `/video/v1/assets/${assetId}`, { passthrough: serializePassthrough(curPt) });
              console.log(`[webhook] Sermon thumbnail set from pre-generated Storage file: ${newUrl}`);
            } else {
              console.log(`[webhook] New sermon VOD ${assetId} — no pre-generated thumb found (${storagePath}); Sunday cron will generate.`);
            }
          } catch (e) {
            console.warn(`[webhook] Sermon thumbnail check failed: ${e.message}`);
          }
        })();
      }

      // ── Deepgram pipeline for live-to-VOD: poll until mp4 rendition ready ──
      // video.asset.live_stream_completed fires when the stream ends and the VOD
      // is created. mp4_support is enabled above, but rendition generation is
      // async. We CANNOT rely solely on video.asset.static_renditions.ready
      // because that event may have already fired for the HLS delivery renditions
      // BEFORE mp4_support was enabled, and Mux may not re-fire it for the new
      // MP4 renditions. Instead, poll here for up to 30 min until audio.m4a
      // is available, then run the full Deepgram pipeline.
      if (isSermon) {
        scheduleSermonDeepgram(assetId).catch(e =>
          console.error(`[live-deepgram] schedule failed for ${assetId}: ${e.message}`));
      }
    } catch (err) {
...
// Firestore has TWO hard limits that bite on large sermons:
//   1. Document size ≤ 1 MB.
//   2. Index entries per document ≤ 20,000. A words array of {w,s,e} objects
//      creates 3 index entries per word — so 7,000+ words hits the cap.
//
// Solution: store words as a JSON STRING (wordsJson) field, not a native array.
//   • Firestore does not index into string contents → no index-entry limit.
//   • String fields are subject only to the 1 MB doc size limit.
//   • If the JSON blob still exceeds ~900 KB we split across numbered chunk
//     docs in a subcollection (wordChunks), each storing a wordsJson slice.
//
// Storage shape:
//   Single-string doc (most sermons, ≤900KB):
//     transcriptWords/{pid} → { wordsJson: "[...]", count, assetId, model, updatedAt }
//   Chunked (very long sermons, >900KB):
//     transcriptWords/{pid} → { chunked: true, chunkCount: N, count, assetId, model, updatedAt }
//     transcriptWords/{pid}/wordChunks/0 → { wordsJson: "[...slice 0]" }
//     transcriptWords/{pid}/wordChunks/1 → { wordsJson: "[...slice 1]" }
//
// Legacy read path (pre-wordsJson docs that stored words: [...] native array)
// is preserved: if wordsJson is absent, fall back to words array field.

const WORDS_CHUNK_BYTES = 900 * 1024; // 900 KB — comfortable under 1 MB limit

// Write words to Firestore, using wordsJson string (avoids index limit) and
// chunking if the payload approaches 1 MB.
async function storeTranscriptWords(db, pid, words, { assetId = null } = {}) {
  const ref = db.collection('transcriptWords').doc(pid);
  const meta = { assetId: assetId || null, model: 'nova-2', count: words.length,
  ...
  if (totalBytes <= WORDS_CHUNK_BYTES) {
    // Single-doc path: store as string to avoid index entry limit.
    await ref.set({ wordsJson: json, ...meta }, { merge: true });
    return;
  }

  // Multi-chunk path: split the JSON blob into slices that each fit under
  // the doc size limit. Slice by word count using proportional estimate.
  const chunks = [];
  let start = 0;
  while (start < words.length) {
    let end = Math.min(words.length, start + Math.ceil(words.length * WORDS_CHUNK_BYTES / totalBytes) + 50);
    // Shrink until the serialized slice fits.
    while (end > start + 1 && Buffer.byteLength(JSON.stringify(words.slice(start, end)), 'utf8') > WORDS_CHUNK_BYTES) end--;
    chunks.push(words.slice(start, end));
    start = end;
  }

  // Write chunk docs first, each as a wordsJson string.
  const chunkColl = ref.collection('wordChunks');
  for (let i = 0; i < chunks.length; i++) {
    await chunkColl.doc(String(i)).set({ wordsJson: JSON.stringify(chunks[i]) });
  }
  // Write the index doc last — chunked:true is the reader signal.
  await ref.set({ chunked: true, chunkCount: chunks.length, ...meta }, { merge: false });
  console.log(`[transcriptWords] ${pid}: stored ${words.length} words in ${chunks.length} chunks (~${Math.round(totalBytes / 1024)}KB)`);
}

// Decode a wordsJson string field, with fallback to legacy words array field.
function decodeWordsField(data) {
  if (!data) return null;
  if (typeof data.wordsJson === 'string' && data.wordsJson.length) {
    try { return JSON.parse(data.wordsJson); } catch { return null; }
  }
  // Legacy fallback: native words array.
  const w = data.words;
...
async function readTranscriptWords(pid) {
  ...
  if (data?.chunked) {
    // Reassemble from subcollection chunks (ordered by numeric doc id string).
    ...
    for (const cs of chunkSnaps.docs) {
      const cw = decodeWordsField(cs.data());
      if (Array.isArray(cw)) all.push(...cw);
    }
    return all.length ? all : null;
  }
  return decodeWordsField(data);
}
...
async function transcriptWordsExist(db, pid) {
  ...
  // Single-doc: check count field (new) or wordsJson (new) or words array (legacy).
  if (typeof d?.count === 'number') return d.count > 0;
  if (typeof d?.wordsJson === 'string') return d.wordsJson.length > 2; // "[]" = 2 chars
...
// After a live-stream-completed event, poll until the mp4 audio rendition is
// ready (up to 30 min), then run the full Deepgram pipeline. This is the
// RELIABLE trigger for live-to-VOD sermon transcripts: video.asset.ready fires
...
async function _scheduleSermonDeepgramInner(assetId) {
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    // Fetch fresh asset (playback id, mp4 status)
    const data = await mux('GET', `/video/v1/assets/${assetId}`);
    const asset = data.data;
    if (!asset) { console.warn(`[live-deepgram] ${assetId}: asset not found on attempt ${attempt}`); break; }
    const pid = asset.playback_ids?.[0]?.id;
    if (!pid) { console.warn(`[live-deepgram] ${assetId}: no playback id yet (attempt ${attempt})`); }
    else {
      // Check if audio.m4a (or low.mp4) is available
      let renditionReady = false;
      for (const f of ['audio.m4a', 'low.mp4', 'medium.mp4']) {
        try {
          const h = await fetch(`https://stream.mux.com/${pid}/${f}`, { method: 'HEAD' });
          if (h.ok) { renditionReady = true; break; }
        } catch { /* keep checking */ }
      }
      if (renditionReady) {
        ...
        // Run Deepgram words + install captions
        console.log(`[live-deepgram] ${assetId} (${pid}): rendition ready on attempt ${attempt} — running Deepgram`);
        const res = await generateAndStoreWords(pid, assetId, { force: false });
        if (res.ok) {
          console.log(`[live-deepgram] ${assetId}: stored ${res.count} words`);
          try { await swapToDeepgramCaptions(pid, assetId); }
          catch (e) { console.warn(`[live-deepgram] caption swap failed: ${e.message}`); }
        } else {
          console.warn(`[live-deepgram] ${assetId}: Deepgram failed — ${res.error}`);
        }
        return;
      }
      console.log(`[live-deepgram] ${assetId}: rendition not ready yet (attempt ${attempt}/${MAX_ATTEMPTS}) — retrying in ${POLL_MS/1000}s`);
    }
    // Wait before next poll (but not after the last attempt)
    if (attempt < MAX_ATTEMPTS) await new Promise(r => setTimeout(r, POLL_MS));
  }
  console.warn(`[live-deepgram] ${assetId}: gave up after ${MAX_ATTEMPTS} attempts — safety-net cron will retry`);
}
...
// ── Universal links: AASA + public watch page ────────────────────────────────
const AASA = {
  applinks: {
    apps: [],
    details: [
      { appIDs: ['N93KX6VC9R.com.gospeloutreacholympia.tv'], components: [{ '/': '/watch/*' }] },
      { appID: 'N93KX6VC9R.com.gospeloutreacholympia.tv', paths: ['/watch/*'] },
    ],
  },
};
...
app.get('/watch/:playbackId', async (req, res) => {
...
<meta name="apple-itunes-app" content="app-id=6764472420, app-argument=https://go-admin-production-6be4.up.railway.app/watch/${pid}">
<meta property="og:title" content="${escapeHtml(title)}">
<meta property="og:image" content="${poster}">
<meta property="og:type" content="video.other">
<title>${escapeHtml(title)} — GO Media</title>
<style>
  ...
  * { box-sizing: border-box; margin: 0; }
  ...
  .appstore b { display: block; font-size: 15px; }
  .appstore span { color: #9a9aa3; font-size: 13px; }
</style>
</head>
<body>
<div class="wrap">
