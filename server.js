...
app.post('/webhooks/mux', express.raw({ type: 'application/json' }), async (req, res) => {
  ...
  if (event.type === 'video.asset.live_stream_completed') {
    ...
    try {
      ...
      // ── Sermon thumbnail: generated by local Mac cron (Sunday PM) ───────────
      // Server-side text rendering is unreliable in this container (fontconfig
      // issues with sharp/Pango on Railway minimal image). Thumbnails are
      // generated by scripts/generate-sermon-thumbs.js on the Mac host and
      // uploaded to Firebase Storage; the Sunday OpenClaw cron calls this.
      // This block just logs a reminder; the cron handles it within ~hours.
      if (catLabel === 'Sermon') {
        console.log(`[webhook] New sermon VOD ${assetId} — thumbnail will be generated by Sunday cron.`);
      }
    } catch (err) {
